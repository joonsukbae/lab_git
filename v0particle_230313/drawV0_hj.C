...
{
    /// @root file load - - - - - - - - - - - - - - - -
    auto mc_file = TFile::Open("../mc/LHC22k3b/hyper/AnalysisResults_mc_hyper_PbPb_LHC22k3b2_with_V0Cuts_8.root", "open");
    auto data_file = TFile::Open("../data/LHC22s/hyper/AnalysisResults_data_hyper_PbPb_LHC22s_apass4_with_v0Cuts_8.root", "open");
    ...
    // / @data object load - - - - - - - - - - - - - - - -
    auto data_dir = (TDirectory *)data_file->Get("multiplicity-counter/Tracks/ProcessCounting");
    data_dir->cd();
    ...
    auto HistMCV0DauEta = mc_v0daueta.GetTH1("HistMCK0ShortDauEta", 2, {kINEL, -1, 1,-1});
    ...
    auto HistDATAreczvtx = data_reczvtx.GetTH1("HistDATAreczvtx", 2, {kDATA, kMBAND, -1});
...
//     // auto HistDATAAntilambdaDauEtaNeg = data_v0daueta.GetTH1("HistDATAAntilambdaDauEtaNeg", 3, {kDATA, kNegative, kAntilambda, -1});
    ...
    auto HistMCV0Count = mc_v0count.GetTH1("HistMCV0Count", 1, {kINEL, -1 , 3});
    // auto HistMCK0ShortV0Count = mc_v0count.GetTH1("HistMCV0Count", 1, {kINEL, kK0short , 2});
    ...
    auto HistDATAV0Count = data_v0count.GetTH1("HistDATAV0Count", 1, {kDATA, -1, 3});
...
//     /// @make histogram - - - - - - - - - - - - - - - -
    ...
    HistMCV0DauEta->Scale(1. / NormalizationValueMC, "width");

    HistMCV0Count->Scale(1. / NormalizationValueMC, "width");
...
//     HistMCAntilambdaDauEtaNeg->Scale(1. / NormalizationValueMC, "width");
    ...
    HistDATAV0Count->Scale(1. / NormalizationValueDATA, "width");
...
// HistMCV0DauEta->Draw("hist][text0");
// auto c1 = new TCanvas("c1","c1",800,600);
// gStyle->SetOptStat(0);
// HistMCV0Count->Draw("hist][text0");
// HistMCV0Count->SetTitle("V0 count after cut (MC)");
// HistMCV0Count->GetXaxis()->SetTitle("Species");
// HistMCV0Count->GetYaxis()->SetTitle("Count [#]");
// HistMCV0Count->GetXaxis()->ChangeLabel(1,-1,-1,-1,-1,-1," ");
// HistMCV0Count->GetXaxis()->ChangeLabel(3,-1,-1,-1,-1,-1," ");
// HistMCV0Count->GetXaxis()->ChangeLabel(5,-1,-1,-1,-1,-1," ");
// HistMCV0Count->GetXaxis()->ChangeLabel(7,-1,-1,-1,-1,-1," ");
// HistMCV0Count->GetXaxis()->ChangeLabel(2,-1,-1,-1,-1,-1,"K^{0}_{s}");
// HistMCV0Count->GetXaxis()->ChangeLabel(4,-1,-1,-1,-1,-1,"#Lambda");
// HistMCV0Count->GetXaxis()->ChangeLabel(6,-1,-1,-1,-1,-1,"#bar{#Lambda}");
// auto c101 = new TCanvas("c101","c101",800,600);
// gStyle->SetOptStat(0);
// HistDATAV0Count->Draw("hist][text0");
// HistDATAV0Count->SetTitle("V0 count after cut (DATA)");
// HistDATAV0Count->GetXaxis()->SetTitle("Species");
// HistDATAV0Count->GetYaxis()->SetTitle("Count [#]");
// HistDATAV0Count->GetXaxis()->ChangeLabel(1,-1,-1,-1,-1,-1," ");
// HistDATAV0Count->GetXaxis()->ChangeLabel(3,-1,-1,-1,-1,-1," ");
// HistDATAV0Count->GetXaxis()->ChangeLabel(5,-1,-1,-1,-1,-1," ");
// HistDATAV0Count->GetXaxis()->ChangeLabel(7,-1,-1,-1,-1,-1," ");
// HistDATAV0Count->GetXaxis()->ChangeLabel(2,-1,-1,-1,-1,-1,"K^{0}_{s}");
// HistDATAV0Count->GetXaxis()->ChangeLabel(4,-1,-1,-1,-1,-1,"#Lambda");
// HistDATAV0Count->GetXaxis()->ChangeLabel(6,-1,-1,-1,-1,-1,"#bar{#Lambda}");
// auto c102 = new TCanvas("c102","c102",800,600);
// gStyle->SetOptStat();
// HistMCK0Shortmass->Draw("e");
// HistDATAK0Shortmass->Draw("samee");
