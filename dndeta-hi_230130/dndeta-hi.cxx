...
using DaughterTracks = soa::Join<aod::TracksIU, aod::TracksCovIU, aod::pidTPCPi, aod::pidTPCKa, aod::pidTPCPr, aod::pidTOFPi, aod::pidTOFPr>;
...
struct MultiplicityCounter {
  ...
  HistogramRegistry registry{
    "registry",
    {
      ...
      {"hV0DauEta", "", {HistType::kTHnSparseD, {EvtClassAxis, SignAxis, EtaAxis}}},                                                                       //
      {"hV0DauEta2", "", {HistType::kTHnSparseD, {EvtClassAxis, SignAxis, EtaAxis}}}                                                                       //
      ...
      if (std::abs(z) < 10) {
        ...
        for (auto& v0 : fullV0s) {
          if (v0.v0radius() > v0radius &&
            ...
            registry.fill(HIST("hV0DauEta"), Double_t(kDATA), Double_t(kPositive), v0.template posTrack_as<FiTracks>().eta());
            registry.fill(HIST("hV0DauEta"), Double_t(kDATA), Double_t(kNegative), v0.template negTrack_as<FiTracks>().eta());
          }
    ...
    for (auto& collision : collisions) {
      ...
      for (auto& v0 : fullV0s) {
        if (v0.v0radius() > v0radius &&
          ...
          registry.fill(HIST("hV0DauEta"), Double_t(kINEL), Double_t(kPositive), v0.posTrack_as<DaughterTracks>().eta());
          registry.fill(HIST("hV0DauEta"), Double_t(kINEL), Double_t(kNegative), v0.negTrack_as<DaughterTracks>().eta());
        }
