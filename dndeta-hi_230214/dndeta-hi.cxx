...
struct MultiplicityCounter {
  ...
  HistogramRegistry registry{
    "registry",
    {
      ...
      {"hCollisionType", "", {HistType::kTHnSparseD, {EvtClassAxis, StepAxis, testAxis}}},                                                                 //
      {"hMCParticleID", "", {HistType::kTHnSparseD, {EvtClassAxis, StepAxis, particleIDAxis}}},                                                            //
      {"hStatusCode", "", {HistType::kTHnSparseD, {EvtClassAxis, StepAxis, StatusCodeAxis}}},                                                              //
      {"hMCStatusCode", "", {HistType::kTHnSparseD, {EvtClassAxis, StepAxis, StatusCodeAxis}}},                                                            //
      {"hProcessCode", "", {HistType::kTHnSparseD, {EvtClassAxis, StepAxis, ProcessCodeAxis}}},                                                            //
      {"hMotherV0Count", "", {HistType::kTHnSparseD, {EvtClassAxis, SpeciesAxis}}},                                                                      //
    ...
    for (auto& collision : collisions) {
      ...
      for (auto& track : tracks) {
        ...
        if (track.has_mcParticle()) {
          ...
          registry.fill(HIST("hMCParticleID"), Double_t(kINEL), Double_t(kAll), track.mcParticleId());

          registry.fill(HIST("hStatusCode"), Double_t(kINEL), Double_t(kAll), track.mcParticle_as<Particles>().getGenStatusCode());
          registry.fill(HIST("hMCStatusCode"), Double_t(kINEL), Double_t(kAll), track.mcParticle_as<Particles>().getHepMCStatusCode());
          ...
          for (auto MotherIDs = track.mcParticle_as<Particles>().mothersIds().front(); MotherIDs <= track.mcParticle_as<Particles>().mothersIds().back(); MotherIDs++) {
            ...
            if (pdg_mother == 3122) {
              registry.fill(HIST("hMotherV0Count"), Double_t(kINEL), Double_t(kLambda));
            }
            if (pdg_mother == -3122) {
              registry.fill(HIST("hMotherV0Count"), Double_t(kINEL), Double_t(kAntilambda));
            }
          }
        } else {
          // when secondary
      ...
      for (auto& particle : particles) {
        // registry.fill(HIST("hStatusCode"), Double_t(kINEL), Double_t(kAll), particle.getGenStatusCode());
        // registry.fill(HIST("hMCStatusCode"), Double_t(kINEL), Double_t(kAll), particle.getHepMCStatusCode());
        // registry.fill(HIST("hProcessCode"), Double_t(kINEL), Double_t(kAll), particle.getProcess());
        // if (particle.producedByGenerator()) {
        //   registry.fill(HIST("hCollisionType"), Double_t(kINEL), Double_t(kAll), 1);
        //   continue;
        // }
        // if (particle.fromBackgroundEvent()) {
        //   registry.fill(HIST("hCollisionType"), Double_t(kINEL), Double_t(kAll), 2);
        //   continue;
        // }
        // if (particle.isPhysicalPrimary()) {
        //   registry.fill(HIST("hCollisionType"), Double_t(kINEL), Double_t(kAll), 3);
        //   continue;
        // }
